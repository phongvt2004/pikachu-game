...
bool LMatching(char ** board, Cell start, Cell end) {
    ...
    return (
        (checkLineX(board, start.y, end.y+1, start.x)
        and checkLineY(board, start.x, end.x, end.y)
        and board[start.x][start.y] == board[end.x][end.y])
        or
        (checkLineY(board, start.x, end.x, start.y)
        and checkLineX(board, start.y+1, end.y, end.x)
        and board[start.x][start.y] == board[end.x][end.y])
    );
}
...
bool checkMatch(Board *board, Cell start, Cell end) {
    ...
    fOut << IMatching(newBoard, start, end) << endl;
    fOut << LMatching(newBoard, start, end) << endl;
    fOut << UMatching(newBoard, start, end) << endl;
    fOut << ZMatching(newBoard, start, end) << endl;
    return match;
}